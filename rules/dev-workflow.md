...
**심볼을 넣는 이유는 난이도가 아니라 혼입 제거다.** 파일 목록만 주면 추론이 두 단계가 된다 —
(1) 파일에 뭐가 들었을지, (2) 그중 어디가 걸릴지. 1번은 구조 감각이 아니라 **그 repo에
얼마나 익숙한가**이고, 시간이 지나면 저절로 올라가 적중률을 감각과 무관하게 밀어올린다.
심볼을 주면 그 단계가 빠지고 재려던 2번만 남는다. **대신 난이도가 내려가므로 형식을 바꾼 날
이전 숫자와는 비교하지 않는다** (2026-08-19 변경).

**심볼 이름이 답을 흘릴 때가 있다.** 루프가 리뷰 지적을 반영하며 새로 만든 함수
(`_handle_...`, `_fix_...`)가 헤더에 뜨면 그게 곧 지적사항이다. 그런 이름이 보이면 그 항목을
빼거나 그날은 `skip` 한다.
...
- **블록 → 표 한 줄로 접는 규칙.** 로그는
  `| 날짜 | 티켓 | 예측 (리뷰 전) | 적중 | 놓친 것 | 판정 | 본 것 (사실) | 근거 |` 8열이다.
  `예측`은 1단계에서 받은 3개를 키워드로 줄여 넣고 (`N+1, 트랜잭션 경계, 타임존`),
  `적중`은 `2/3`. 1단계를 건너뛴 날은 세 칸 다 `—`.
