...
불통과 신호는 **발화 여부를 반드시 명시**한다. plan이 "이 값이 나오면 의심하라"고 미리
적어둔 것이라, 발화했는데 안 보여주면 게이트가 존재할 이유가 없다.
(실례 — #89 plan: *"`unknown`이 0건이면 의심한다"* → 실제 `unknown` 0건 발화 →
`SYMBOLS_VERIFICATION.md:169-176`에서 코드 경로 + 전수 42건으로 해명. 검증 출력 7줄에는
이 수치가 들어갈 칸이 아예 없다.)

plan에 통과 기준·불통과 신호가 **비어 있으면 그 사실을 첫 줄에 적는다**
...
- **블록 → 표 한 줄로 접는 규칙.** 로그는 `| 날짜 | 티켓 | 판정 | 본 것 (사실) | 근거 |` 5열이다.
  위 블록 전체를 넣지 말고 `본 것` 칸에는 **기준 요약 + 발화한 신호만** 적는다:
  `기준 5/5 ✓ · ⚠unknown 0(VERIFICATION.md:169)` / `기준 3/5 ✓, 4번 미측정 · 신호 없음`.
  통과한 신호와 검증 출력 7줄은 칸에서 뺀다 — 블록은 판단할 때 보는 화면이고 표는 나중에 다시
