...
# Safety net: any unexpected error → silently approve (never block commands)
trap 'exit 0' ERR
set -uo pipefail
...
# Skip absolute-path commands (/usr/bin/curl, /usr/bin/git, etc.)
# User is deliberately bypassing RTK
case "$FIRST_CMD" in
  /usr/*|/bin/*|/opt/*) exit 0 ;;
esac
...
if echo "$FIRST_CMD" | grep -qE '^git\s+status(\s|$)'; then
  # Skip RTK when piped/redirected or targeting specific paths (-- separator)
  if ! echo "$CMD" | grep -qE '\||>' && ! echo "$CMD" | grep -qF ' -- '; then
...
# git branch - only rewrite simple listing, skip create/delete/rename/piped/merged
elif echo "$FIRST_CMD" | grep -qE '^git\s+branch(\s|$)' \
  && ! echo "$FIRST_CMD" | grep -qE '^git\s+branch\s+(-[dDmMcCuU]|--delete|--move|--copy|--set-upstream|--unset-upstream)' \
  && ! echo "$FIRST_CMD" | grep -qE '^git\s+branch\s+[A-Za-z0-9_/]' \
  && ! echo "$FIRST_CMD" | grep -qE '^git\s+branch\s+--?(merged|no-merged)' \
  && ! echo "$CMD" | grep -qF '|'; then
